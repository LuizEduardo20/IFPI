...
Operacao.prototype.validate = function () {
	...
	let tipoDeOperacao = this.data.tipoOperacao;
...
Operacao.prototype.create = function () {
	return new Promise((resolve, reject) => {
		const query_sql = 'INSERT INTO operacoes (data, ativo, tipo_de_operacao, quantidade, preco, valor_bruto, taxa_b3, valor_liquido) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id;'
		const dataOperacao = new Date()
		const values = [dataOperacao, this.data.ativo, this.data.tipoDeOperacao, this.data.quantidade, this.data.preco, this.data.valorBruto, this.data.taxaB3, this.data.valorLiquido]
...
Operacao.prototype.readUserOperations = function () {
	return new Promise((resolve, reject) => {
		const query_sql = 'SELECT data, ativo, tipo_de_operacao, quantidade, preco, valor_bruto, taxa_b3, valor_liquido FROM operacoes WHERE usuario_id = $1';
...
Operacao.prototype.update = function () {
	return new Promise((resolve, reject) => {
		const query_sql = 'UPDATE operacoes SET data = $1, ativo = $2, tipo_de_operacao = $3, quantidade = $4, preco = $5, valor_bruto = $6, taxa_b3 = $7, valor_liquido = $8 WHERE id = $9';
		const dataOperacao = new Date()
		const values = [dataOperacao, this.data.ativo, this.data.tipoDeOperacao, this.data.quantidade, this.data.preco, this.data.valorBruto, this.data.taxaB3, this.data.valorLiquido, this.idDaOperacao];
		pool.query(query_sql, values, (error, result) => {
			if (error) {
...
Operacao.prototype.delete = function () {
	return new Promise((resolve, reject) => {
		const query_sql = 'DELETE FROM operacoes WHERE id = $1';
		const values = [this.idDaOperacao];
		pool.query(query_sql, values, (error, result) => {
			if (error) {
