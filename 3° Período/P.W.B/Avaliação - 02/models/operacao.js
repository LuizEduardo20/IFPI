...
Operacao.prototype.readById = function () {
    return new Promise((resolve, reject) => {
        const query_sql = 'SELECT id, data, ativo, tipo_de_operacao, quantidade, preco, valor_bruto, taxa_b3, valor_liquido FROM operacoes WHERE id = $1 AND usuario_id = $2';
        const values = [this.idDaOperacao, this.idDoUsuario];
        pool.query(query_sql, values, (error, result) => {
            if (error) {
                reject(`Erro ao recuperar operação: ${error}`);
            } else {
                ...
                const operacao = {
                    ...
                    preco: Number(tupla.preco),
                    valorBruto: Number(tupla.valor_bruto),
                    taxaB3: Number(tupla.taxa_b3),
                    valorLiquido: Number(tupla.valor_liquido)
                };
                resolve(operacao);
            }
        });
